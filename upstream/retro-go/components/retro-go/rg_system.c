...
static char holo_app_rom_path[HOLO_MODULE_PATH_MAX];
#endif
...
static void apply_holo_launch(void)
{
    ...
    strncpy(holo_app_config_ns, launch.config_ns, sizeof(holo_app_config_ns) - 1);
    holo_app_config_ns[sizeof(holo_app_config_ns) - 1] = 0;
    strncpy(holo_app_rom_path, launch.rom_path, sizeof(holo_app_rom_path) - 1);
    holo_app_rom_path[sizeof(holo_app_rom_path) - 1] = 0;

    app.configNs = holo_app_config_ns;
    app.bootArgs = holo_app_rom_path;
    app.bootFlags = launch.boot_flags;
    app.saveSlot = (app.bootFlags & RG_BOOT_SLOT_MASK) >> 4;
    app.romPath = app.bootArgs ?: "";
    app.isLauncher = strcmp(app.configNs, "launcher") == 0;
}
#endif
...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
...
#if defined(RG_TARGET_HOLO_DYNMOD)
    tasks[0] = (rg_task_t){.handle = (TaskHandle_t)&tasks[0], .name = "main"};
#else
    ...
    rg_task_create("rg_sysmon", &system_monitor_task, NULL, 4 * 1024, RG_TASK_PRIORITY_5, -1);
...
#if defined(RG_TARGET_HOLO_DYNMOD)
    if (!handle)
        return rg_task_find("rg_display");
#endif
    for (size_t i = 0; i < RG_COUNT(tasks); ++i)
    {
...
{
#if defined(RG_TARGET_HOLO_DYNMOD)
    RG_LOGW("Exit requested; switching to holo launcher");
...
#if defined(RG_TARGET_HOLO_DYNMOD)
    (void)partition;
    holo_runtime_request_switch(name && name[0] ? name : "launcher", args ? args : "", flags);
    return;
#endif
