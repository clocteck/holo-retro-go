...
static void gwenesis_profiler_maybe_log(void)
{
        ...
        async_vdp_discards)
    {
        ...
        RG_LOGI("gwen vdp0: sub=%u drop=%u ren=%u disp=%u disc=%u snap=%uus render=%uus",
...
static bool gwenesis_frameskip_should_draw(bool audio_enabled, int64_t now)
{
    ...
    const bool draw = frameskip_fixed_phase < 2;
    frameskip_fixed_phase = (frameskip_fixed_phase + 1) % 3;
    (void)audio_enabled;
    return draw;
