...
- 模块加载路径：当前 app 目录下的 `modules`，例如 `/sd/apps/holo-retro-go/modules/retrogo.so`
- ROM 根目录：当前 app 目录下的 `roms`，例如 `/sd/apps/holo-retro-go/roms`
- ROM 上传页面：当前 app 的 WebUI 路由，例如 `/holo-retro-go/`
...
- Lua app 负责扫描当前 app 目录下的 `roms`（例如 `/sd/apps/holo-retro-go/roms`）并把 catalog 注入模块，模块侧的 Retro-Go storage 接口再读取这个虚拟目录列表。Lua app 还注册了一个简单 WebUI，可通过浏览器上传 ROM 到该目录。
