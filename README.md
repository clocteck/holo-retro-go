...
- `upstream/retro-go` 保存官方 Retro-Go 源码。
- `main/retrogo_module.c` 提供动态模块导出入口：`module_query_v1`、`module_create_v1`、`module_luaopen_v1`、`module_destroy_v1`。
- `main/holo_*.c` 提供 Holocubic 宿主 API 到 Retro-Go 运行环境的兼容层。
- Lua app 负责扫描 `/sd/roms` 并把 catalog 注入模块，模块侧的 Retro-Go storage 接口再读取这个虚拟目录列表。
