...
- `nes`: Holocubic classic Retro-Go core collection. Output: `src/build/retrogo.so`; package snapshot: `package/modules/retrogo.so`.
...
- `gwenesis`: Holocubic Sega Mega Drive / Genesis only. Output: `src/build-gwenesis/gwenesis.so`; package snapshot: `package/modules/gwenesis.so`.

Device module paths:

```text
/sd/apps/holo-retro-go/modules/retrogo.so
/sd/apps/holo-retro-go/modules/gwenesis.so
```
...
```text
/sd/apps/holo-retro-go/main.lua
```

ROM root on the device:

```text
/sd/apps/holo-retro-go/roms
```

## Build Environment

- Target chip: ESP32-S3
- Workspace path: `E:\cubicsrc\APPS\holo-retro-go\src`
...
Run build commands from `holo-retro-go/src`. Reconfigure when CMake files or profile selection changed:
...
Upload `gwenesis.so` from the package snapshot:
...
Upload the Lua app entry from the package:
...
- Gwenesis also allocates hot VDP state/buffers with `MEM_FAST`: VRAM, CRAM, CRAM565, VSRAM, SAT cache, VDP registers/FIFO, and the per-line plane/sprite render buffers. Async VDP VRAM snapshots stay in `MEM_SLOW` to fit the Holo internal RAM budget. These are released by the MD cleanup path.
...
- Gwenesis keeps the 64KB M68K RAM in `MEM_SLOW` because it is too large for the remaining internal RAM budget on Holo dynmod builds. It mirrors 4 hot 4KB M68K RAM pages in `MEM_FAST`, defaulting to pages `a,d,e,f`. Prefer keeping smaller hot blocks in `MEM_FAST`: Z80 RAM, VDP registers/CRAM/SAT/fifo/line buffers, YM/SN source buffers, the mixed audio buffer, and the audio ring.
- Gwenesis audio uses `MEM_FAST` for the YM2612/SN76489 source buffers, a mixed stereo buffer, and the audio ring. Current Holo dynmod `ymc:1` build keeps `gwen_audio`/YM compute and the merged host-audio drain on Core 1; async VDP and `rg_display` stay on Core 0. VDP async uses `GWENESIS_VDP_ASYNC_JOBS=2` and a latest-frame/drop policy: no READY/RENDERING backlog, no hot-path sync fallback, and no waiting for CPU0 to go idle. SN76489 stays off by default and the YM-only path bypasses the mixer for performance; enable SN76489 only when PSG accuracy matters more than speed.
