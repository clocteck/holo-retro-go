...
void nes_main(void)
{
    ...
    autocrop = rg_settings_get_number(NS_APP, SETTING_AUTOCROP, 1);
    palette = rg_settings_get_number(NS_APP, SETTING_PALETTE, NES_PALETTE_NTSC);
    ...
    while (true)
    {
...
#if defined(RG_TARGET_HOLO_DYNMOD)
        bool drawFrame = !nsfPlayer && holoFramePhase < NES_HOLO_RENDER_FRAMES;
#else
...
#if defined(RG_TARGET_HOLO_DYNMOD)
        if (!nsfPlayer)
            holoFramePhase = (holoFramePhase + 1) % NES_HOLO_FRAME_CYCLE;
#endif
        ...
        if (skipFrames == 0)
        {
            ...
            if (nsfPlayer)
                ...
                skipFrames = (++frameSkipPhase % 3) ? 0 : 1;
