...
typedef enum
{
    GWENESIS_AUDIO_MODE_ACCURATE = 0,
    GWENESIS_AUDIO_MODE_BALANCE = 1,
} gwenesis_audio_mode_t;

static gwenesis_audio_mode_t gwenesis_audio_mode = GWENESIS_AUDIO_MODE_ACCURATE;

static const char *gwenesis_audio_mode_name(gwenesis_audio_mode_t mode)
{
    switch (mode)
    {
    case GWENESIS_AUDIO_MODE_ACCURATE:
        return "Accurate";
...
#define GWENESIS_AUDIO_RING_SUBMIT_FRAMES 85
#define GWENESIS_AUDIO_RING_TARGET_FRAMES 700
#define GWENESIS_AUDIO_RING_HIGH_FRAMES 900
#define GWENESIS_AUDIO_RING_BLOCK_FRAMES 933
#define GWENESIS_AUDIO_HIGH_WATER_PAUSE_MS 3
#define GWENESIS_AUDIO_BLOCK_WAIT_MS 2
#define GWENESIS_AUDIO_OUTPUT_TASK_STACK (3 * 1024)
...
#define GWENESIS_VDP_SNAPSHOT_FULL_THRESHOLD_PAGES \
    ((48U * 1024U) / GWENESIS_VDP_SNAPSHOT_PAGE_SIZE)
#define GWENESIS_VDP_EVENT_CAPACITY 4096U
#define GWENESIS_VDP_EVENT_FAST_CAPACITY 256U
#define GWENESIS_VDP_EVENT_DATA_CAPACITY (256U * 1024U)
#define GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY 1024U
#define GWENESIS_VDP_EVENT_MEMCPY_MIN 16U
#endif
// --- MAIN
...
} gwenesis_sync_event_t;

static gwenesis_sync_event_t gwenesis_audio_data_event;
static gwenesis_sync_event_t gwenesis_audio_space_event;
#endif
...
static gwenesis_sync_event_t gwenesis_vdp_work_event;
#endif
...
static inline int gwenesis_z80_batch_lines_current(bool yfm_run_enabled)
{
    ...
    return GWENESIS_Z80_BATCH_LINES_ACCURATE;
}
...
static inline int gwenesis_ym_batch_lines_current(void)
{
    ...
    return GWENESIS_YM_BATCH_LINES_ACCURATE;
}
...
#define GWENESIS_PROFILER_SAMPLE_INTERVAL_FRAMES 8U
static uint32_t gwenesis_profiler_frame_sequence;
static bool gwenesis_profiler_sample_frame;
...
{
    return gwenesis_perf_overlay_enabled && gwenesis_profiler_sample_frame;
}

static inline void gwenesis_profiler_begin_frame(void)
{
    gwenesis_profiler_sample_frame =
        gwenesis_perf_overlay_enabled &&
        (gwenesis_profiler_frame_sequence++ % GWENESIS_PROFILER_SAMPLE_INTERVAL_FRAMES) == 0;
}
#endif

#if defined(RG_TARGET_HOLO_DYNMOD)
static const module_sync_api_t *gwenesis_sync_api(void)
{
    const module_host_api_v2 *host = holo_port_host();
    if (!host || !host->sync.create_counting || !host->sync.take ||
        !host->sync.give || !host->sync.destroy)
        return NULL;
    return &host->sync;
}

static bool gwenesis_sync_event_create(gwenesis_sync_event_t *event)
{
    const module_sync_api_t *sync = gwenesis_sync_api();
    if (!event || !sync)
        return false;
    if (event->handle)
        return true;
    return sync->create_counting(1, 0, &event->handle) == MODULE_OK && event->handle;
}

static void gwenesis_sync_event_signal(gwenesis_sync_event_t *event)
{
    const module_sync_api_t *sync = gwenesis_sync_api();
    if (event && event->handle && sync)
        (void)sync->give(event->handle);
}

static bool gwenesis_sync_event_wait(gwenesis_sync_event_t *event,
                                     uint32_t timeout_ms,
                                     uint32_t fallback_delay_ms)
{
    const module_sync_api_t *sync = gwenesis_sync_api();
    if (event && event->handle && sync)
        return sync->take(event->handle, timeout_ms) == MODULE_OK;
    rg_task_delay(fallback_delay_ms);
    return false;
}

static void gwenesis_sync_event_destroy(gwenesis_sync_event_t *event)
{
    const module_sync_api_t *sync = gwenesis_sync_api();
    if (!event || !event->handle)
        return;
    if (sync)
        sync->destroy(event->handle);
    event->handle = NULL;
}
#endif
...
static inline uint32_t gwenesis_profiler_centi_fps(uint32_t frames, int64_t elapsed_us)
{
    if (elapsed_us <= 0)
        return 0;

    return (uint32_t)(((uint64_t)frames * 100000000ULL +
                       (uint64_t)elapsed_us / 2ULL) /
                      (uint64_t)elapsed_us);
}

static inline uint32_t gwenesis_profiler_smooth_centi_fps(uint32_t previous,
                                                           uint32_t sample)
{
    /* 3/4 previous + 1/4 new: stable display with a small once-per-update cost. */
...
static void gwenesis_profiler_maybe_log(void)
{
    ...
    const int64_t elapsed_us = now - gwenesis_profiler.last_log_us;
    if (elapsed_us < 1000000 ||
        gwenesis_profiler.profiled_frames == 0)
        return;
    ...
    const int m68k_avg_us = gwenesis_profiler_avg_us(gwenesis_profiler.m68k_us,
                                                     gwenesis_profiler.profiled_frames);
    ...
    const uint32_t fps100 =
        gwenesis_profiler_centi_fps(gwenesis_profiler.frames, elapsed_us);
    const uint32_t draw_fps100 =
        gwenesis_profiler_centi_fps(gwenesis_profiler.draw_frames, elapsed_us);
    if (gwenesis_perf_overlay.fps_valid)
    {
        gwenesis_perf_overlay.fps100 =
            gwenesis_profiler_smooth_centi_fps(gwenesis_perf_overlay.fps100, fps100);
        gwenesis_perf_overlay.draw_fps100 =
            gwenesis_profiler_smooth_centi_fps(gwenesis_perf_overlay.draw_fps100,
                                               draw_fps100);
    }
    else
    {
        gwenesis_perf_overlay.fps100 = fps100;
        gwenesis_perf_overlay.draw_fps100 = draw_fps100;
        gwenesis_perf_overlay.fps_valid = true;
    }
...
static void gwenesis_audio_wait_below_block_water(void)
{
    while (gwenesis_audio_task_running && gwenesis_audio_block_water_now())
        (void)gwenesis_sync_event_wait(&gwenesis_audio_space_event,
...
static size_t gwenesis_audio_ring_drain(bool force, bool fade_in)
{
    ...
    const size_t count = fill >= GWENESIS_AUDIO_RING_SUBMIT_FRAMES
    ...
    gwenesis_audio_eq_process_frames(gwenesis_audio_ring_drain_buffer, count);
    if (fade_in)
        gwenesis_audio_apply_fade_in(gwenesis_audio_ring_drain_buffer, count);
    ...
    gwenesis_sync_event_signal(&gwenesis_audio_space_event);
    return count;
}
...
static size_t gwenesis_audio_ring_write_frames(const rg_audio_frame_t *frames, size_t count)
{
    ...
    size_t offset = 0;
    while (offset < count && gwenesis_audio_task_running)
    {
        ...
        gwenesis_sync_event_signal(&gwenesis_audio_data_event);
        offset += copy_count;
    }
    return offset;
}
...
static void gwenesis_audio_output_task(void *arg)
{
    ...
    while (gwenesis_audio_task_running || gwenesis_audio_ring_fill() > 0)
    {
        ...
        if (fill == 0)
        {
            ...
            prebuffering = true;
            (void)gwenesis_sync_event_wait(&gwenesis_audio_data_event, 10, 1);
            continue;
        }

        if (gwenesis_audio_task_running && prebuffering && fill < GWENESIS_AUDIO_RING_TARGET_FRAMES)
        {
            (void)gwenesis_sync_event_wait(&gwenesis_audio_data_event, 10, 1);
            continue;
        }
        ...
        const size_t count = gwenesis_audio_ring_drain(!gwenesis_audio_task_running,
                                                       was_underflowing);
        if (count > 0)
        {
            last_frame = gwenesis_audio_ring_drain_buffer[count - 1];
...
static bool gwenesis_audio_start(void)
{
    ...
    const bool audio_data_sync_enabled = gwenesis_sync_event_create(&gwenesis_audio_data_event);
    const bool audio_space_sync_enabled = gwenesis_sync_event_create(&gwenesis_audio_space_event);
    if (!audio_data_sync_enabled || !audio_space_sync_enabled)
        RG_LOGW("Genesis audio sync unavailable; falling back to delay polling\n");
    gwenesis_audio_task_running = true;
    gwenesis_audio_output_task_handle =
        rg_task_create("gwen_aout", gwenesis_audio_output_task, NULL,
    ...
    if (!gwenesis_audio_output_task_handle)
    {
        ...
        gwenesis_sync_event_destroy(&gwenesis_audio_data_event);
        gwenesis_sync_event_destroy(&gwenesis_audio_space_event);
        ...
        RG_LOGW("Genesis audio output task unavailable, using direct audio\n");
    }
    else
    {
        RG_LOGI("Genesis audio output task started on core %d, ring=%d target=%d "
                "ring_mem=MEM_FAST drain_mem=MEM_FAST wait=data-%s/space-%s\n",
                GWENESIS_AUDIO_OUTPUT_TASK_CORE,
                GWENESIS_AUDIO_RING_FRAMES,
                GWENESIS_AUDIO_RING_TARGET_FRAMES,
                audio_data_sync_enabled ? "signal" : "poll",
                audio_space_sync_enabled ? "signal" : "poll");
    }
#endif
...
static void gwenesis_audio_submit_frames(rg_audio_frame_t *frames, size_t count)
{
    ...
    if (gwenesis_audio_output_task_handle)
    {
        const size_t written = gwenesis_audio_ring_write_frames(frames, count);
        if (written == count)
            return;
        frames += written;
        count -= written;
...
static void gwenesis_audio_submit_frame(size_t count, bool psg_enabled)
{
    if (rg_audio_get_mute())
        return;
    if (!gwenesis_audio_mix_buffer || count == 0)
        return;
...
static void gwenesis_perf_overlay_draw(rg_surface_t *surface)
{
    ...
    const uint32_t fps = (gwenesis_perf_overlay.fps100 + 50U) / 100U;
    const uint32_t draw_fps = (gwenesis_perf_overlay.draw_fps100 + 50U) / 100U;
    snprintf(line, sizeof(line), "F%02u D%02u",
             (unsigned)(fps > 99 ? 99 : fps),
             (unsigned)(draw_fps > 99 ? 99 : draw_fps));
...
typedef struct
{
    ...
    uint8_t line;
    uint8_t type;
} gwenesis_vdp_event_t;
typedef char gwenesis_vdp_event_size_must_be_12[(sizeof(gwenesis_vdp_event_t) == 12) ? 1 : -1];
...
typedef struct
{
    ...
    unsigned char *regs;
    unsigned char *event_hot_storage;
    gwenesis_vdp_event_t *events_fast;
    gwenesis_vdp_event_t *events_slow;
    unsigned char *event_data_fast;
    unsigned char *event_data_slow;
...
#endif
/* Only the emulation task accesses this pointer. Publishing a completed job
 * to the render task is synchronized through the job state release/acquire. */
static gwenesis_vdp_async_job_t *gwenesis_vdp_async_recording_job;
static gwenesis_vdp_palette_remap_t *gwenesis_vdp_palette_remap;
...
static inline gwenesis_vdp_async_job_t *gwenesis_vdp_async_event_job(void)
{
    gwenesis_vdp_async_job_t *job = gwenesis_vdp_async_recording_job;
    if (!job || scan_line < 0 || scan_line >= job->screen_height)
        return NULL;
    return job;
}

static inline gwenesis_vdp_event_t *gwenesis_vdp_async_event_at(
    gwenesis_vdp_async_job_t *job, uint32_t index)
{
    if (index < GWENESIS_VDP_EVENT_FAST_CAPACITY)
        return &job->events_fast[index];
    return &job->events_slow[index - GWENESIS_VDP_EVENT_FAST_CAPACITY];
}

static inline void gwenesis_vdp_async_event_data_store(gwenesis_vdp_async_job_t *job,
                                                        uint32_t offset,
                                                        unsigned char value)
{
    if (offset < GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY)
        job->event_data_fast[offset] = value;
    else
        job->event_data_slow[offset - GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY] = value;
}

static inline unsigned char gwenesis_vdp_async_event_data_load(
    const gwenesis_vdp_async_job_t *job, uint32_t offset)
{
    if (offset < GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY)
        return job->event_data_fast[offset];
    return job->event_data_slow[offset - GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY];
}

static void gwenesis_vdp_async_event_data_copy(const gwenesis_vdp_async_job_t *job,
                                                uint32_t offset,
                                                ...
                                                size_t length)
{
    while (length > 0)
    {
        ...
        if (offset < GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY)
        {
            source = job->event_data_fast + offset;
            chunk = GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY - offset;
        }
        else
        {
            const uint32_t slow_offset = offset - GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY;
            source = job->event_data_slow + slow_offset;
            chunk = GWENESIS_VDP_EVENT_DATA_CAPACITY - offset;
        }
        if (chunk > length)
            chunk = length;
...
static void gwenesis_vdp_async_record_scalar(gwenesis_vdp_event_type_t type,
                                              ...
                                              unsigned int value)
{
    ...
    {
        gwenesis_vdp_event_t *last =
            gwenesis_vdp_async_event_at(job, job->event_count - 1);
        if (last->type == (uint8_t)type && last->line == (uint8_t)scan_line &&
            last->address == (uint16_t)address && last->count == 1)
        {
            last->data = value;
            return;
        }
    ...
    if (job->event_count >= GWENESIS_VDP_EVENT_CAPACITY)
    {
        job->event_overflow = true;
        return;
    }

    gwenesis_vdp_event_t *event = gwenesis_vdp_async_event_at(job, job->event_count++);
    event->data = value;
    event->line = (uint8_t)scan_line;
...
{
    gwenesis_vdp_async_record_scalar(GWENESIS_VDP_EVENT_REG8, reg, value & 0xFFU);
}
...
void gwenesis_vdp_async_record_cram565(unsigned int index, unsigned int value)
{
    gwenesis_vdp_async_job_t *job = gwenesis_vdp_async_event_job();
    if (job)
        job->remap_palette = true;
    gwenesis_vdp_async_record_scalar(GWENESIS_VDP_EVENT_CRAM565, index, value & 0xFFFFU);
}
...
void gwenesis_vdp_async_record_sat_vram(unsigned int address, unsigned int value)
{
    ...
    if (job->event_count > 0)
    {
        last = gwenesis_vdp_async_event_at(job, job->event_count - 1);
        if (last->line != (uint8_t)scan_line || last->count == UINT16_MAX)
        {
            last = NULL;
        }
        else if (last->type == fill_type)
        {
            const uint16_t expected =
                (uint16_t)(last->address + (int32_t)last->stride * last->count);
            if ((unsigned char)last->data == byte_value &&
                expected == (uint16_t)address)
            {
                last->count++;
                return;
            }
        ...
        else if (last->type != run_type ||
                 last->data + last->count != job->event_data_size)
        {
            last = NULL;
        }
    }
    ...
    {
        const int16_t stride = (int16_t)((uint16_t)address - last->address);
        if (stride == 0)
        {
            gwenesis_vdp_async_event_data_store(job, last->data, byte_value);
            return;
        }

        const unsigned char first_value =
            gwenesis_vdp_async_event_data_load(job, last->data);
        if (first_value == byte_value)
        {
            /* The only byte owned by this event is the newest event-data byte,
             * so it can be reclaimed when the run becomes an inline fill. */
            job->event_data_size--;
            last->data = byte_value;
            last->stride = stride;
            last->count = 2;
            last->type = fill_type;
            return;
    ...
    {
        if (job->event_count >= GWENESIS_VDP_EVENT_CAPACITY ||
            job->event_data_size >= GWENESIS_VDP_EVENT_DATA_CAPACITY)
        {
            job->event_overflow = true;
            return;
        }
        last = gwenesis_vdp_async_event_at(job, job->event_count++);
        last->data = job->event_data_size;
        last->line = (uint8_t)scan_line;
        last->address = (uint16_t)address;
        last->count = 0;
        last->stride = 0;
        last->type = run_type;
    }
    else if (job->event_data_size >= GWENESIS_VDP_EVENT_DATA_CAPACITY)
    {
        job->event_overflow = true;
        return;
    }

    gwenesis_vdp_async_event_data_store(job, job->event_data_size++, byte_value);
    last->count++;
}
...
static gwenesis_vdp_async_job_t *gwenesis_vdp_async_take_oldest_ready_job(void)
{
...
{
#if defined(RG_TARGET_HOLO_DYNMOD)
    gwenesis_sync_event_signal(&gwenesis_vdp_work_event);
#endif
}
...
static bool gwenesis_vdp_async_begin_snapshot(uint32_t frame_id, int frame_width, int frame_height)
{
    ...
    job->event_count = 0;
    job->event_data_size = 0;
    job->event_overflow = false;
    job->remap_palette = false;
    ...
    __atomic_store_n(&job->state, GWENESIS_VDP_JOB_RECORDING, __ATOMIC_RELEASE);
    gwenesis_vdp_async_recording_job = job;
    return true;
}

static void gwenesis_vdp_async_seal_snapshot(uint32_t frame_id)
{
    gwenesis_vdp_async_job_t *job = gwenesis_vdp_async_recording_job;
    gwenesis_vdp_async_recording_job = NULL;
    if (!job || job->frame_id != frame_id)
        return;

    if (job->event_overflow)
    {
        /* Keep the frame renderable without replaying an incomplete timeline. */
        memcpy(job->vram, VRAM, VRAM_MAX_SIZE);
        memcpy(job->vsram, VSRAM, sizeof(*VSRAM) * VSRAM_MAX_SIZE);
        memcpy(job->cram565, CRAM565, sizeof(*CRAM565) * CRAM_MAX_SIZE * 4);
        memcpy(job->sat_cache, SAT_CACHE, SAT_CACHE_MAX_SIZE);
        ...
        job->event_data_size = 0;
        job->remap_palette = false;
        RG_LOGW("Genesis VDP event log overflow frame=%u; rendering final snapshot",
                (unsigned)frame_id);
    }
...
static uint8_t gwenesis_vdp_async_palette_intern(gwenesis_vdp_palette_remap_t *remap,
                                                  ...
                                                  uint8_t fallback)
{
    ...
    {
        mapped = remap->color_count++;
        remap->output_palette[mapped] = color;
        return (uint8_t)mapped;
    }
...
static void gwenesis_vdp_async_palette_remap_begin(gwenesis_vdp_async_job_t *job)
{
    if (!job->remap_palette || !gwenesis_vdp_palette_remap)
        return;

    gwenesis_vdp_palette_remap_t *remap = gwenesis_vdp_palette_remap;
    memset(remap, 0, sizeof(*remap));
    memset(job->surface->palette, 0, sizeof(*job->surface->palette) * 256);
    remap->output_palette = job->surface->palette;

    /* Build the complete source-index LUT once from the frame-start CRAM.
     * CRAM565 contains four 64-entry mirrors, so this normally interns at
     * most 64 colors instead of doing validation and lookup for every pixel. */
    for (unsigned int source = 0; source < 256; ++source)
    {
        remap->source_map[source] =
            gwenesis_vdp_async_palette_intern(remap, job->cram565[source], 0);
    }
}

static inline unsigned int gwenesis_vdp_async_job_sat_address(
    const gwenesis_vdp_async_job_t *job)
{
    const unsigned int mask = (job->regs[12] & 0x01U) ? 0x7EU : 0x7FU;
    return (job->regs[5] & mask) << 9;
}

static inline unsigned int gwenesis_vdp_async_job_sat_size(
    const gwenesis_vdp_async_job_t *job)
{
    return (job->regs[12] & 0x01U) ? 0x400U : 0x200U;
}
...
static void gwenesis_vdp_async_apply_events(gwenesis_vdp_async_job_t *job,
                                             int line,
                                             uint32_t *event_index)
{
    while (*event_index < job->event_count)
    {
        const gwenesis_vdp_event_t *event =
            gwenesis_vdp_async_event_at(job, *event_index);
        if (event->line > (uint8_t)line)
            break;

        switch ((gwenesis_vdp_event_type_t)event->type)
        {
        case GWENESIS_VDP_EVENT_REG8:
        ...
        case GWENESIS_VDP_EVENT_CRAM565:
            ...
            {
                const unsigned short pixel = (unsigned short)event->data;
                job->cram565[event->address] = pixel;
                job->cram565[0x40 + event->address] = pixel;
                job->cram565[0x80 + event->address] = pixel;
                job->cram565[0xC0 + event->address] = pixel;

                if (job->remap_palette && gwenesis_vdp_palette_remap &&
                    gwenesis_vdp_palette_remap->output_palette)
                {
                    gwenesis_vdp_palette_remap_t *remap = gwenesis_vdp_palette_remap;
                    const uint8_t mapped = gwenesis_vdp_async_palette_intern(
                        remap, pixel, remap->source_map[event->address]);
                    remap->source_map[event->address] = mapped;
                    remap->source_map[0x40 + event->address] = mapped;
                    remap->source_map[0x80 + event->address] = mapped;
                    remap->source_map[0xC0 + event->address] = mapped;
                }
            }
            break;
        case GWENESIS_VDP_EVENT_VSRAM16:
            if (event->address < VSRAM_MAX_SIZE)
                job->vsram[event->address] = (unsigned short)event->data;
            break;
        }
...
#if defined(RG_TARGET_HOLO_DYNMOD)
static gwenesis_vdp_async_job_t *gwenesis_vdp_async_take_oldest_done_job(void)
{
    gwenesis_vdp_async_job_t *best = NULL;
    if (!gwenesis_vdp_async_jobs)
        return NULL;
...
static void gwenesis_vdp_async_worker_task(void *arg)
{
    ...
    {
        gwenesis_vdp_async_job_t *done = gwenesis_vdp_async_take_oldest_done_job();
        if (done && gwenesis_vdp_async_display_job_nonblocking(done))
        {
            __atomic_store_n(&done->state, GWENESIS_VDP_JOB_HELD, __ATOMIC_RELEASE);
            continue;
        }

        gwenesis_vdp_async_job_t *job = gwenesis_vdp_async_take_oldest_ready_job();
        if (!job)
        {
#if defined(RG_TARGET_HOLO_DYNMOD)
            (void)gwenesis_sync_event_wait(&gwenesis_vdp_work_event,
                                           done ? 2 : 10, 2);
#else
            rg_task_delay(2);
#endif
        ...
        gwenesis_vdp_render_context_t ctx = {
            ...
            .palette_remap = job->remap_palette ? gwenesis_vdp_palette_remap : NULL,
        };
        ...
        gwenesis_vdp_async_palette_remap_begin(job);

        uint32_t event_index = 0;
#if GWENESIS_PROFILER_DETAILED
        const bool profile_render = gwenesis_perf_overlay_enabled;
        const int64_t render_start_us = profile_render ? rg_system_timer() : 0;
#endif
        ...
        for (int line = 0; line < job->screen_height; ++line)
        {
            if (__atomic_load_n(&job->discard, __ATOMIC_ACQUIRE))
                break;
            gwenesis_vdp_async_apply_events(job, line, &event_index);
        ...
        if (profile_render)
        {
            gwenesis_vdp_async_render_us += (uint32_t)(rg_system_timer() - render_start_us);
...
#if defined(RG_TARGET_HOLO_DYNMOD)
            if (job->remap_palette && gwenesis_vdp_palette_remap &&
                gwenesis_vdp_palette_remap->overflow)
                RG_LOGW("Genesis VDP dynamic palette exhausted frame=%u",
                        (unsigned)job->frame_id);
    ...
    while (true)
    {
        ...
        for (int i = 0; i < GWENESIS_VDP_ASYNC_JOBS; ++i)
        {
            ...
            if (state == GWENESIS_VDP_JOB_FILLING ||
                state == GWENESIS_VDP_JOB_RECORDING ||
            ...
            else if (state == GWENESIS_VDP_JOB_PREPARING)
            {
                __atomic_store_n(&job->state, GWENESIS_VDP_JOB_FREE, __ATOMIC_RELEASE);
            }
        }
...
static bool gwenesis_vdp_async_start(void)
{
    ...
    bool event_hot_fallback_used = false;

    if (!gwenesis_vdp_palette_remap)
        gwenesis_vdp_palette_remap =
            rg_alloc(sizeof(*gwenesis_vdp_palette_remap), MEM_FAST | MEM_NOPANIC);
    if (!gwenesis_vdp_palette_remap)
    {
        RG_LOGW("Genesis VDP async disabled: palette remap allocation failed\n");
        return false;
    }
    ...
    memset(gwenesis_vdp_async_jobs, 0,
           sizeof(*gwenesis_vdp_async_jobs) * GWENESIS_VDP_ASYNC_JOBS);
    for (int i = 0; i < GWENESIS_VDP_ASYNC_JOBS; ++i)
    {
        gwenesis_vdp_async_job_t *job = &gwenesis_vdp_async_jobs[i];
        ...
        const size_t event_hot_size =
            sizeof(*job->events_fast) * GWENESIS_VDP_EVENT_FAST_CAPACITY +
            GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY;
        job->event_hot_storage = rg_alloc(event_hot_size, MEM_FAST | MEM_NOPANIC);
        if (!job->event_hot_storage)
        {
            job->event_hot_storage = rg_alloc(event_hot_size, MEM_SLOW | MEM_NOPANIC);
            event_hot_fallback_used = job->event_hot_storage != NULL;
        }
        job->events_fast = (gwenesis_vdp_event_t *)job->event_hot_storage;
        job->event_data_fast = job->event_hot_storage
                                   ? job->event_hot_storage +
                                         sizeof(*job->events_fast) * GWENESIS_VDP_EVENT_FAST_CAPACITY
                                   : NULL;
        job->events_slow = rg_alloc(
            sizeof(*job->events_slow) *
                (GWENESIS_VDP_EVENT_CAPACITY - GWENESIS_VDP_EVENT_FAST_CAPACITY),
            MEM_SLOW | MEM_NOPANIC);
        job->event_data_slow = rg_alloc(
            GWENESIS_VDP_EVENT_DATA_CAPACITY - GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY,
            MEM_SLOW | MEM_NOPANIC);
        if (!job->vram || !job->vsram || !job->cram565 || !job->sat_cache || !job->regs ||
            !job->event_hot_storage || !job->events_slow || !job->event_data_slow)
        {
...
#if defined(RG_TARGET_HOLO_DYNMOD)
    __atomic_store_n(&gwenesis_vdp_async_paused, false, __ATOMIC_RELEASE);
    const bool vdp_sync_enabled = gwenesis_sync_event_create(&gwenesis_vdp_work_event);
    if (!vdp_sync_enabled)
        RG_LOGW("Genesis VDP sync unavailable; falling back to delay polling\n");
#endif
...
#if defined(RG_TARGET_HOLO_DYNMOD)
    RG_LOGI("Genesis VDP worker wait=%s", vdp_sync_enabled ? "signal" : "poll");
#endif
    RG_LOGI("Genesis VDP event log: events=%u fast=%u hot=%uB/job total_hot=%uKB data=%uKB fallback=%d",
            (unsigned)GWENESIS_VDP_EVENT_CAPACITY,
            (unsigned)GWENESIS_VDP_EVENT_FAST_CAPACITY,
            (unsigned)(sizeof(gwenesis_vdp_event_t) * GWENESIS_VDP_EVENT_FAST_CAPACITY +
                       GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY),
            (unsigned)((sizeof(gwenesis_vdp_event_t) * GWENESIS_VDP_EVENT_FAST_CAPACITY +
                        GWENESIS_VDP_EVENT_DATA_FAST_CAPACITY) *
                       GWENESIS_VDP_ASYNC_JOBS / 1024),
            (unsigned)(GWENESIS_VDP_EVENT_DATA_CAPACITY / 1024),
            event_hot_fallback_used ? 1 : 0);
    return true;
}
...
static void gwenesis_vdp_async_stop(void)
{
    ...
    gwenesis_vdp_async_recording_job = NULL;
#if defined(RG_TARGET_HOLO_DYNMOD)
    gwenesis_sync_event_destroy(&gwenesis_vdp_work_event);
    if (gwenesis_vdp_async_jobs)
    {
#endif
    for (int i = 0; i < GWENESIS_VDP_ASYNC_JOBS; ++i)
    {
        ...
        free(job->sat_cache);
        free(job->regs);
        free(job->event_hot_storage);
        free(job->events_slow);
        free(job->event_data_slow);
        job->vram = NULL;
        job->vsram = NULL;
        job->cram565 = NULL;
        job->sat_cache = NULL;
        job->regs = NULL;
        job->event_hot_storage = NULL;
        job->events_fast = NULL;
        job->events_slow = NULL;
        job->event_data_fast = NULL;
        job->event_data_slow = NULL;
        job->surface = NULL;
        __atomic_store_n(&job->state, GWENESIS_VDP_JOB_FREE, __ATOMIC_RELEASE);
    }
#if defined(RG_TARGET_HOLO_DYNMOD)
...
#endif
    free(gwenesis_vdp_palette_remap);
    gwenesis_vdp_palette_remap = NULL;
}
#else
void gwenesis_vdp_async_mark_midframe_write(void)
{
...
static rg_gui_event_t audio_mode_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    {
        gwenesis_audio_mode = gwenesis_audio_mode == GWENESIS_AUDIO_MODE_ACCURATE
                                   ? GWENESIS_AUDIO_MODE_BALANCE
                                   : GWENESIS_AUDIO_MODE_ACCURATE;
...
static rg_gui_event_t perf_overlay_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
    {
        ...
        if (gwenesis_perf_overlay_enabled)
        {
...
void app_main(void)
{
...
#if GWENESIS_AUDIO_EMULATION
    gwenesis_audio_mode = rg_settings_get_number(NS_APP, SETTING_AUDIO_MODE, GWENESIS_AUDIO_MODE_ACCURATE);
    if (gwenesis_audio_mode < GWENESIS_AUDIO_MODE_ACCURATE ||
        gwenesis_audio_mode > GWENESIS_AUDIO_MODE_BALANCE)
    {
        gwenesis_audio_mode = GWENESIS_AUDIO_MODE_BALANCE;
    }
    ...
    RG_LOGI("mod audio: synth=%dHz output=%dHz buffer=%d ring=%d target=%d high=%d block=%d submit=%d highwait=%dms blockwait=%dms\n",
            ...
            GWENESIS_AUDIO_RING_BLOCK_FRAMES,
            GWENESIS_AUDIO_RING_SUBMIT_FRAMES,
    ...
    while (true)
    {
        ...
        if (drawFrame)
        {
            ...
            {
                async_vdp_path = gwenesis_vdp_async_begin_snapshot(async_vdp_frame_id,
                                                                   screen_width,
                                                                   screen_height);
                if (!async_vdp_path)
                    drawFrame = false;
            ...
            if (gwenesis_vdp_async_can_submit_frame())
            {
                ...
                const bool async_vdp_submitted =
                    gwenesis_vdp_async_begin_snapshot(async_vdp_frame_id,
                                                      screen_width,
                                                      screen_height);
                if (!async_vdp_submitted)
                    async_vdp_path = false;
            }
            else
            {
                gwenesis_vdp_async_wait_idle_for_sync();
        ...
        while (scan_line < lines_per_frame)
        {
            ...
            if (gwenesis_profiler_active())
            {
                z80_profiler_begin_m68k();
                m68k_run(line_target);
                const int64_t nested_z80_us = z80_profiler_end_m68k();
                const int64_t m68k_wall_us = rg_system_timer() - prof_start;
                gwenesis_profiler.z80_us += nested_z80_us;
                gwenesis_profiler.m68k_us += m68k_wall_us > nested_z80_us
                                                  ? m68k_wall_us - nested_z80_us
                                                  : 0;
            }
            else
#endif
        ...
        if (async_vdp_path)
        {
            gwenesis_vdp_async_seal_snapshot(async_vdp_frame_id);
#if defined(GWENESIS_VDP_ASYNC_SERIAL_TEST) && GWENESIS_VDP_ASYNC_SERIAL_TEST
        ...
        if (gwenesis_perf_overlay_enabled)
        {
            ...
            if (gwenesis_profiler_active())
                gwenesis_profiler.profiled_frames++;
            if ((gwenesis_profiler.frames % GWENESIS_PROFILER_SAMPLE_INTERVAL_FRAMES) == 0)
